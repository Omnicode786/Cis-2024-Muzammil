...
const spritePaths: Record<SpriteKey, string> = {
  ...
  talismanShot: '/assets/game/talisman-fire-seal.svg',
...
export default function SignalBreach() {
  ...
  useEffect(() => {
    ...
    const spawnPickup = () => {
      ...
      const novaOnScreen = pickupsRef.current.some((pickup) => pickup.kind === 'nova');
      const pressureCount = hazardsRef.current.filter((hazard) => hazard.x > 42 && hazard.x < gameWidth + 150).length;
      const phaseTwoPressure = phaseRef.current === 2 && pressureCount >= (levelRef.current >= 42 ? 11 : 8);
      ...
      const kind: Pickup['kind'] =
        !mysteryOnScreen && (forceMystery || roll < mysteryChance)
          ? 'mystery'
          : phaseTwoPressure && !novaOnScreen
            ? 'nova'
          : phaseRef.current === 2 && roll < 0.13
          ? 'cache'
          : canRepair && roll < 0.24
          ? 'repair'
          : player.shield <= 0 && roll < 0.34
            ? 'shield'
            : levelRef.current >= 2 && roll < (phaseRef.current === 2 ? 0.56 : 0.42)
              ? 'nova'
              : roll < 0.58
                ? 'overclock'
                : 'shard';
    ...
    const spawnBossHorde = (boss: BossChallenge) => {
      if (hazardsRef.current.length >= maxHazards + 16) return;
    ...
    const updateBossChallenge = (delta: number) => {
      ...
      if (boss.phase === 'strike') {
        ...
        if (hazardsRef.current.length > maxHazards + 18) {
          hazardsRef.current = hazardsRef.current.slice(-(maxHazards + 18));
        }
    ...
    const update = (delta: number) => {
      ...
      const visibleHazardPressure = hazardsRef.current.filter((hazard) => hazard.x > 34 && hazard.x < gameWidth + 120).length;
      const needsPhaseTwoEmberRelief = phase === 2 && level >= phaseOneFinalBoss && visibleHazardPressure >= (level >= 48 ? 13 : 9);
      if (spawnFreezeRef.current > 0) {
        hazardTimerRef.current = Math.max(hazardTimerRef.current, 34);
      }
      if (needsPhaseTwoEmberRelief && !pickupsRef.current.some((pickup) => pickup.kind === 'nova')) {
        pickupTimerRef.current = Math.min(pickupTimerRef.current, 16);
        hazardTimerRef.current = Math.max(hazardTimerRef.current, 18);
      }

      if (hazardTimerRef.current <= 0 && spawnFreezeRef.current <= 0) {
        if (hazardsRef.current.length < maxHazards) {
          spawnHazard();
      ...
      if (pickupTimerRef.current <= 0) {
        spawnPickup();
        pickupTimerRef.current = Math.max(phase === 2 ? 26 : 42, random(phase === 2 ? 52 : 82, phase === 2 ? 94 : 132) - level * (phase === 2 ? 0.82 : 1.08));
      }
      ...
      if (bossRef.current?.active) return;
      hazardsRef.current = hazardsRef.current.filter((hazard) => hazard.x > -150);
      if (hazardsRef.current.length > maxHazards + 12) {
        hazardsRef.current = hazardsRef.current.slice(-(maxHazards + 12));
      }
    ...
    const drawProjectile = (projectile: Projectile) => {
      ...
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.globalCompositeOperation = projectile.kind === 'laser' ? 'lighter' : 'source-over';
      ...
      if (projectile.kind === 'fire') {
        drawSprite('talismanShot', 0, 0, 46 + Math.sin(projectile.phase * 1.4) * 2.5, Math.sin(projectile.phase) * 0.08, alpha);
        ctx.strokeStyle = `rgba(95,58,34,${0.42 * alpha})`;
        ctx.lineWidth = 2;
        ctx.lineCap = 'round';
        [-1, 0, 1].forEach((lane) => {
          ctx.beginPath();
          ctx.moveTo(-20, lane * 7);
          ctx.bezierCurveTo(-36, lane * 8 + Math.sin(projectile.phase + lane) * 6, -58, lane * 4, -76, lane * 9);
          ctx.stroke();
        });
        ctx.fillStyle = `rgba(185,74,54,${0.62 * alpha})`;
