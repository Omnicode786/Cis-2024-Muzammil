...
import { apiError, badRequest, conflict, forbidden, notFound, unauthorized } from "@/lib/api-response";
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const supplier = await prisma.supplier.findFirst({ where: { id: params.id, shopId: user.shopId }, include: { _count: { select: { purchases: true } } } });
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (supplier._count.purchases > 0) return badRequest("Cannot delete a supplier with existing purchases. Delete those records first.");
