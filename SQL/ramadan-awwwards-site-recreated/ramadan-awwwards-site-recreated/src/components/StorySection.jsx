  ...
  return (
    <section ref={ref} className="relative px-6 py-20 md:px-8 lg:px-12 lg:py-28">
      <div className="mx-auto max-w-7xl section-grid">
        ...
        >
          <p className="text-xs uppercase tracking-[0.45em] text-[var(--theme-accent-strong)]">{eyebrow}</p>
          <h3 className="mt-5 font-display text-4xl leading-none text-[var(--theme-text)] md:text-5xl">
            {title}
          </h3>
          <p className="mt-6 max-w-2xl text-base leading-8 text-[var(--theme-muted)] md:text-lg">{body}</p>
        </motion.div>
        ...
        <motion.div
          ...
          className={`theme-card relative overflow-hidden rounded-[2rem] p-8 md:p-10 ${
        ...
        >
          ...
          <div className="relative">
            <p className="text-sm uppercase tracking-[0.45em] text-[var(--theme-subtle)]">Chapter {index + 1}</p>
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              {stats.map((stat, statIndex) => (
                <motion.div
                  ...
                  className="theme-chip rounded-[1.5rem] p-5"
                >
                  <div className="mb-4 h-1 w-12 rounded-full bg-gradient-to-r from-amber-200 to-amber-400" />
                  <p className="font-display text-2xl tracking-wide text-[var(--theme-text)]">{stat}</p>
                </motion.div>
              ))}
            </div>
          </div>
